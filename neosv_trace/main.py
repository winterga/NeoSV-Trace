...
def main():
    ...
    if not args.anno:
        ...
        if sv_fusions:
            ...
            mhc_predict_run(args.mhc_predictor, args.netmhc, args.mhcflurry,
                            file_mhc_in, hla_alleles, file_mhc_out)
        else:
            open(file_mhc_in, 'w').close()
            open(file_mhc_out, 'w').close()

        # reload and filter the neoepitopes based on netMHC result
        dict_epitope = mhc_predict_reload(args.mhc_predictor, file_mhc_out)
