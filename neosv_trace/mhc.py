...
def mhcflurry_run(mhcflurry_path, peppath, alleles, outpath, input_csv_path=None, log_path=None):
    """
    Run mhcflurry-predict using an INPUT.csv (allele, peptide) and write RESULT.csv via --out.

    INPUT.csv format required by mhcflurry-predict:
      columns: allele, peptide (optionally n_flank, c_flank) :contentReference[oaicite:1]{index=1}
    """

    print("Running MHCflurry (CSV input mode).")

    allele_list = [a.strip() for a in alleles.split(',') if a.strip()]

    # read peptides (one per line) from peppath
    with open(peppath, "r") as f:
        peptides = [line.strip() for line in f if line.strip()]

    if not peptides or not allele_list:
        open(outpath, "w").close()
        return

    # default input/log paths
    if input_csv_path is None:
        input_csv_path = outpath + ".input.csv"
    if log_path is None:
        log_path = outpath + ".log"

    # write the cartesian product allele x peptide to INPUT.csv
    ...
    # run mhcflurry-predict INPUT.csv --out RESULT.csv
    cmd = [mhcflurry_path, input_csv_path, "--out", outpath]

    # capture logs separately (prevents progress output polluting stdout)
    with open(log_path, "w") as logf:
        p = subprocess.run(cmd, stdout=logf, stderr=logf, text=True)

    if p.returncode != 0:
        raise RuntimeError(f"mhcflurry-predict failed (exit {p.returncode}). See log: {log_path}")
...
def mhcflurry_run(mhcflurry_path, peppath, alleles, outpath, chunk_size=500):
    ...
    print(f"Total peptides to process: {len(peptides)}")
...
def mhc_predict_run(predictor, netmhc_path, mhcflurry_path, peppath, alleles, outpath):
    if predictor == 'netmhcpan':
        netmhc_run(netmhc_path, peppath, alleles, outpath)
    elif predictor == 'mhcflurry':
        mhcflurry_run(mhcflurry_path, peppath, alleles, outpath)
    else:
